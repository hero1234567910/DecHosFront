  ...
  <div style="overflow: auto;height:100%;overflow-x: hidden;background-color: #ecf5ff;" id="th">
    <el-card class="box-card" :body-style="cardStyle">
      <div slot="header" class="clearfix">
        ...
        <span style="font-weight: 700;">人员推广记录</span>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <!-- <span>推广次数 :  </span>
    ...
    <div class="weui-cells" style="background-color: #f8fcff;margin-top: 10px;">
      ...
      >
        <div class="weui-cell__bd">
          <div class="weui-cell" @click="toPopu(item)">
            <div class="weui-cell__bd">{{item.userName}}</div>
            <div class="weui-cell__ft">
              <p>{{item.extensionCount}} 次</p>
            </div>
          </div>
        </div>
  ...
  methods: {
    toPopu(ele) {
      this.$router.push({
        ...
        query: {
          promotersGuid: ele.rowGuid,
          count: ele.extensionCount,
    ...
    init() {
      ...
      $("#th").on("infinite", function() {
        if (loading) return;
        loading = true;
        console.log("page=="+(self.page+20));
        setTimeout(function() {
          var params = new URLSearchParams();
          params.append("offset", self.page+20);
          params.append("limit", "20");
          console.log(self.page);
          self.model.getPopuListByAdmin(params).then(function(res) {
            if (res.data.code == "0") {
    ...
    getList(page, limit) {
      ...
      $.showLoading();
      this.model.getPopuListByAdmin(params).then(function(res) {
        $.hideLoading();
        if (res.data.code == 0) {
